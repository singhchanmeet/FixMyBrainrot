...
- lowercase alphabetic single words from 5 to 15 letters
...
- a build-time English frequency band using `wordfreq` (Zipf frequency 3.5–4.19) to avoid both extremely common and genuinely obscure entries
- runtime selection weighted toward less-common and medium-frequency words, with a smaller share of very common words

The dictionary page shows the selected definition and example sentence. Entries are internally grouped into less-common (Zipf 3.5–3.79), medium-frequency (3.8–4.09), and common-within-the-learning-range (4.1–4.19) bands. The activity samples those bands at approximately 50%, 35%, and 15%, respectively. The generated asset is bundled locally, so the activity does not need a dictionary API at runtime.
...
`assets/words/anagrams.js` is generated from the Wordset-derived candidates during the same build. It contains groups of distinct 5–12 letter words with the same letter signature. The anagram pool allows a broader frequency range, including common words, while retaining the same structural and label-based quality filters. A small explicit exclusion list removes poor puzzle candidates such as proper-name-like entries, a trademark, a medical term, and an offensive term. The activity offers Easy (5–6 letters), Medium (7 letters), and Hard (8–10 letters) modes.

`assets/words/anagram-answers.js` is generated as a separate answer index. It includes additional quality-filtered Wordset words that share the signatures of playable puzzles, so legitimate alternatives such as `loader` and `reload` are both accepted even when one is not selected as the displayed puzzle word. Only a short success message is shown; definitions and examples are not loaded by the activity.
