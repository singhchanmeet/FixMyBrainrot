...
function nextProblem() { $('#math-problem').textContent = makeProblem($('#math-difficulty').value); $('#math-answer').value = ''; $('#math-answer').focus(); }
function checkAnswer() { if ($('#math-answer').value !== '' && Number($('#math-answer').value) === answer) nextProblem(); }
function appendKey(key) { const input = $('#math-answer'); if (key === 'clear') input.value = ''; else if (key === 'backspace') input.value = input.value.slice(0, -1); else input.value += key; input.dispatchEvent(new Event('input', { bubbles: true })); input.focus(); }
